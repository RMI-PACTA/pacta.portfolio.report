class techexposure {
  ...
  constructor(container, data, labels,opts) {
    ...
    hover_over_low_carbon = (typeof labels.hover_over_low_carbon === 'undefined') ? {before_sec: "Low-carbon ", after_sec: " technologies"} : labels.hover_over_low_carbon,
    lowCarbonLabel = (typeof labels.green_legend_label === 'undefined') ? {top: "Low-carbon technologies", bottom: "within a sector"} : labels.green_legend_label;
    ...
    function change_class() {
      ...
      let benchmark_names = d3.map(subdata.filter(d => !d.this_portfolio), d => d.portfolio_name_translation).keys().sort();
    ...
    function update() {
      ...
      subdata = subdata.filter(d => d.this_portfolio == true | d.portfolio_name_translation == benchmark_selector.value);
      ...
      x.range([0, width])
      .domain([0, (+percent_selector.value == 1 ? 1 : max_techshare_perc_aum)]).nice();
      ...
      legend_group.append("g").attr("class", "sector_labels_legend")
        ...
        if (subdata.find(datapoint => datapoint.green == true) != undefined) {
          ...
          let number_rectangles_tech = tech_in_prev_sectors[tech_in_prev_sectors.length - 1] + legend_data.filter((obj) => obj.ald_sector_translation === unique_sectors[unique_sectors.length - 1]).length;
          green_legend_group.attr("transform","translate(" + (width + margin.left + 20) + "," + (5 + number_rectangles_tech * 17) + ")");

          green_legend_group.selectAll("rect")
          ...
          green_legend_group.selectAll("text")
    ...
    function mouseover_green(d) {
      tooltip
        .html(hover_over_low_carbon.before_sec + tech_id2name(d.ald_sector) + hover_over_low_carbon.after_sec + "<br>" +
